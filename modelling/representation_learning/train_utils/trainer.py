...
tr_loader, val_loader , tst_loader = get_dataloaders(data_config=data_config)
...
wandb_logger = WandbLogger(project= f"SensorRepresentationLearning", name = model.model_name)
...
trainer = Trainer(callbacks=[early_stop_callback, checkpoint_callback, rich_progress_bar, rich_model_summary, lr_monitor],
                accelerator = train_config['accelerator'] ,max_epochs=train_config['MAX_EPOCHS'], logger=[wandb_logger, csv_logger] ,
